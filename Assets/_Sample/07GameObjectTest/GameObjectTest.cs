...
namespace Sample
{
    public class GameObjectTest : MonoBehaviour
    {
        ...
        //    부모 게임오브젝트를 만들어서 묶은 다음 부모 오브젝트에 접근하여 자식 오브젝트들의 객체를 가져온다
        ...
        void Start()
        {
            //(1) 게임오브젝트에 스크립트 소스 컴포넌트를 추가하여 직접(this.) 가져온다
            //this.transform
            //this.gameObject
            ...
            //(5) ParentObject.chilCount, parentObject.GetChild 반환값으로
            ...
            //(6) 싱글톤 패턴 디자인 : 클래스이름.객체이름으로 접근하여 사용
...
/*
(Hirearchy 창에 있는) 게임오브젝트의 gameobject, transform에 접근하는 방법
...
5) 같은 종류, 기능들로 묶어서 게임 오브젝트의 객체(인스턴스) 가져오기
   부모 게임오브젝트를 만들어서 묶은 다음 부모 오브젝트에 접근하여 자식 오브젝트들의 객체를 가져온다
   ParentObject.chilCount, parentObject.GetChild 반환값으로 가져오기
